...
また、本稿ではほかの言語のジェネリクスやそれに類する機能との比較は行いません。
...
@<code>{int}型を要素としてもつ@<code>{Vector}型の変数を定義したい場合には、@<list>{typeparam_type_typearg}のように型引数として@<code>{int}型を指定することで行なえます。
...
コントラクトはほかのトップレベルで定義した型や関数と同様に、名前が大文字で始まる場合はエクスポートされ、そうでない場合はエクスポートされません。
...
たとえば、@<code>{func compare(T)(a, b []T){/* 略 */}}のような関数定義の場合、引数@<code>{a}と@<code>{b}の型として型パラメータ@<code>{T}が2回登場しています。
...
型パラメータ@<code>{From}については、型の単一化で2回登場しますが、共に@<code>{int}型に対応しているため問題ありません。そのため、@<code>{type_inference_Slice}の関数呼び出しは、型推論により型引数として@<code>{int}と@<code>{string}を指定したことになります。
...
@<code>{int}型が型パラメータ@<code>{T}と対応しているため、@<code>{pair.New(int)(1, 2)}のように呼び出している場合と同等だと判断されます。
...
型推論は型パラメータにコントラクトが指定しているかどうかにかかわらずに行われます。
まず型推論が行われ型引数として暗黙的に指定される型が決定され、その後その型がコントラクトを満たしているか調べられます。

型推論に成功したとしてもコンパイラは実引数が仮引数に割当て可能かチェックします。
