...
Goのエラーハンドリングは何度か話題にあがっています。Go2に向けて「Proposal: Go 2 Error Inspection@<fn>{go2_error}」が提案されていますが、Go1向けのExperimental Packageとしてxerrors（golang.org/x/xerrors）がリリースされています。

//footnote[go2_error][@<href>{https://go.googlesource.com/proposal/+/master/design/29934-error-values.md}]

コールスタックの実行中サブルーチンのエラーを呼び出し元へ情報を失わずにエラー情報を伝播させていくかが開発者の間で考えられていて、pkg/errorsパッケージなどの外部ライブラリが利用されていましたが、Go2では標準ライブラリとしてエラートレースできるように検討されています。
...
Goのログパッケージで有名なものとして、様々なフォーマットをプラグインとしてサポートしているlogrus@<fn>{logrus}や高速にログを書き込めるzap@<fn>{zap}、ログレベルを提供したシンプルなglog@<fn>{glog}などがあります。開発しているアプリケーションの要件に合うログパッケージを選び効率良いロギングを行いましょう。
...
本章では、Webアプリケーションのセキュリティレベルを向上させるためのセキュアコーディングプラクティスをGoのコードを用いて解説しました。セキュアコーディングプラクティスの概念はGoに依存しない、全てのプログラミング言語で活用できるガイドラインです。セキュリティの対応は後手に回れば回るほど問題が発生したときに生じる被害が甚大なものになるので、コーディングをするときに少しでも本章を思い出してか実装をしていってもらえればと思います。
