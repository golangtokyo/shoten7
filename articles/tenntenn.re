...
@<list>{typeliteral_typeinstantiating}のように曖昧さ残る場合があります。

//list[typeliteral_typeinstantiating][型リテラルの末尾に型パラメタを用いた場合におけるインスタンス化の曖昧さ][go]{
x1 := []T(v1)
x2 := []T(v2){}
//}
...
このような曖昧さを避けるため、型リテラルの末尾における型パラメタの
インスタンス化は@<code>{[](T(v1))}のように@<code>{()}で括る必要があります。

@<code>{()}で括らない場合は、@<code>{[]T(x)}は@<code>{[](T(x))}ではなく、
@<code>{([]T)(x)}と解釈されます。
