...
トップレベルでしか定義できないように制限することで、Go1の後方互換性を保ったままにすることが可能です。

キーワードが単純に増えてしまうと既存のコードにその文字列が変数名などの識別子として用いられていた場合にコンパイルエラーになります。
...
一方で新コントラクトのDesign Docでは、ジェネリクスの値はボックス化はされないようになっています。
例えば、@<list>{setter_Strings}のように文字列を引数に取る@<code>{Set}メソッドを有するような型に限定する@<code>{setter}コントラクトを定義したとします。
...
例えば、@<list>{Pair}のように@<code>{Pair}型を定義した場合、@<code>{pair.Pair(int, string)}のように型引数を指定すると、ボックス化されず、@<code>{struct { f1 int; f2 string }}として定義した場合と同様に扱われます。
...
例えば、@<list>{call_Print_int}のように型引数で指定した型のスライス表示する関数@<code>{Print}について考えてみます。

//list[call_Print_int][Print関数に型引数としてint型を指定した場合][go]{
Print(int)([]int{1, 2, 3})
//}

この場合の型引数@<code>{int}は、引数@<code>{[]int{1, 2, 3}}から推論することができます。
型引数の推論は関数のすべての型パラメタが引数として使用されている場合に限ります。
戻り値として使用されていたり、関数本体で使用されているだけでは推論することはできません。
例えば、前述した@<code>{from.Strings}関数は、型パラメタを引数ではなく戻り値に用いているため、関数呼び出し時に推論することはできません。
...
関数定義側で複数同じ型パラメータが現れる場合には、関数呼び出し元でも対応する型が複数回現れる必要があります。
例えば、@<code>{func compare(T)(a, b []T){/* 略 */}}のような関数定義の場合、引数@<code>{a}と@<code>{b}の型として型パラメータ@<code>{T}が2回登場しています。
...
型なしの定数がない場合や対応する型が見つかっていない型パラメータがない場合は型の単一化が終了します。
そうでない場合は、型なしの定数のデフォルトの型（@<code>{100}の場合は@<code>{int}型など）を用いて、最初のパスと同じアルゴリズムで型パラメータを対応させていきます。

例えば、@<list>{type_inference_Print}のように@<code>{Print}関数を呼び出した場合を考えます。
@<code>{[]int}型と@<code>{[]T}型が比較され、@<code>{int}型と型パラメータ@<code>{T}が対応させることができるので型の単一化に成功します。
...
2つの型パラメータ@<code>{From}と@<code>{To}は入力引数として用いられているため、@<list>{type_inference_Slice}のように型推論を行うことが可能です。

//list[type_inference_Slice][Slice関数の型推論を伴った呼び出し][go]{
strs := transform.Slice([]int{1, 2, 3}, strconv.Itoa)
//}

型の単一化により、@<code>{[]int}型と@<code>{[]From}から@<code>{int}型と型パラメータ@<code>{From}が対応します。
また、@<code>{strconv.Itoa}、つまり@<code>{func(int) string}型と@<code>{func(From) To}から@<code>{int}型と型パラメータ@<code>{From}および@<code>{string}と型パラメータ@<code>{To}が対応することが分かります。

型パラメータ@<code>{From}については、型の単一化で2回登場しますが、ともに@<code>{int}型に対応しているため問題ありません。そのため、@<code>{type_inference_Slice}の関数呼び出しは、型推論により型引数として@<code>{int}と@<code>{string}を指定したことになります。
...
@<code>{int}型が型パラメータ@<code>{T}と対応しているため、@<code>{pair.New(int)(1, 2)}のように呼び出してる場合と同等だと判断されます。

@<code>{pair.New(1, int64(2))}のように呼び出した場合、それぞれの実引数は@<code>{1}は型なしの定数、@<code>{int64(2)}は@<code>{int64}型の定数となります。
型の単一化の最初のパスでは、@<code>{int64(2)}から@<code>{int64}型と型パラメータ@<code>{T}が対応することが分かります。
最初のパスが終了した後、対応する型が見つかっていない型パラメータがないため、型の単一化は終了します。
...
@<code>{int}型と@<code>{float64}型が型パラメータ@<code>{T}と対応しているため、矛盾が発生し型の単一化は失敗し、コンパイルエラーになります。

型推論は型パラメータにコントラクトが指定しているかどうかに関わらずに行われます。
まず型推論が行われ型引数として暗黙的に指定される型が決定され、その後その型がコントラクトを満たしているか調べられます。

型推論に成功したとしてもコンパイラは実引数が仮引数に割当可能かどうかチェックします。
これは型なしの定数を実引数として取った場合に限る話ではありません。

型推論は非常に便利な機能ですが、型パラメータやコントラクトの設計に影響を与えるものではなく、単に利便性のために存在します。そのため、型パラメータやコントラクトに関する実装（ランタイムやコンパイラ）の初期段階では省略される可能性があります。

関数呼び出し時の型推論の他に@<list>{type_inference_composite_literal}のようなコンポジットリテラルにおける型推論を考えることもできます。しかし、このようなコードがどのくらいの頻度で必要になるのかは分からないため、実装されない可能性があります。
